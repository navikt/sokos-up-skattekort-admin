...
export default function NeiValg({fnr, inntektsaar, abonnementer}: Readonly<NeiValgProps>) {
    const [abonner, setAbonner] = useState<Set<string>>(new Set);
    const [failed, setFailed] = useState<Set<string>>(new Set);

    return (<HStack>
        {"Nei("}
        <SimpleOpprettAbonnementModal fnr={fnr} inntektsaar={inntektsaar} abonnementer={[...abonnementer, ...abonner]} setAbonner={setAbonner} setFailed={setFailed}/>
        {")"}
